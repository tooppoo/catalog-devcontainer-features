...
check_supported_distribution() {
    ...
    case "${distribution} ${distribution_like}" in
        *debian*|*ubuntu*) ;;
        *)
            echo_stderr "Unsupported distribution: git-utils Feature v0 supports Debian/Ubuntu based images only."
...
install_packages() {
    if ! command -v apt-get >/dev/null 2>&1; then
        echo_stderr "Unsupported distribution: apt-get is required for git-utils Feature v0."
        exit 1
    fi
...
normalize_git_utils_version() {
    ...
    case "${version}" in
        "")
            echo_stderr "Invalid git-utils version: version must not be empty."
            exit 1
            ;;
        latest)
            printf '%s\n' "latest"
...
run_installer() {
    ...
    echo "Installing git-utils from ${INSTALLER_URL}"
